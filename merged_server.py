#!/usr/bin/env python3
"""
LMArena Bridge - Merged Server
Combines API Server and Dashboard Server functionality
"""
...
# ============================================================================
# DASHBOARD SERVER ROUTES (from dashboard_server.py)
# ============================================================================
...
if __name__ == "__main__":
    logger.info("🚀 LMArena Bridge Merged Server starting...")
    logger.info("   - API Server: http://127.0.0.1:5102")
    logger.info("   - Dashboard: http://127.0.0.1:5102/dashboard")
    logger.info("   - WebSocket: ws://127.0.0.1:5102/ws")
    logger.info("   - API docs: http://127.0.0.1:5102/docs")
    ...
    try:
        ...
        if all([admin_username, admin_email, admin_password]):
            conn = db.get_db_connection()
            cursor = conn.cursor()
            cursor.execute("SELECT COUNT(*) as count FROM users")
            user_count = cursor.fetchone()['count']
        ...
        logger.error(f"Failed to create admin user from environment: {e}")
